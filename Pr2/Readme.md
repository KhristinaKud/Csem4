...
```
- bstack(gstack немає у freeBSD, тому я зробила заміну аналогом)
```text
...
## Порівнняння 
- bstack
  * підходить лише для швидкого перегляду стеку
  * показує стек кожного потоку окремо
  * показує список функцій і де вони розташованні в пам'яті
- GDB
  * підходить для більш глибокого аналізу
  * дає змогу аналізувати окремі потоки вручну
  * можна переглядати змінні і робити покадрове дослідження коду
---
# Завдання 2.5
## Опис програми 
Відомо, що при виклику процедур і поверненні з них процесор використовує стек.Чи можна в такій схемі обійтися без лічильника команд (IP), використовуючи замість нього вершину стека? Обґрунтуйте свою
відповідь та наведіть приклади.

  - Що таке лічильник команд(IP)
   Це спеціальний регістр, який відстежує адресу пам'яті наступної інструкції, що має бути виконана в програмі. [Джерело](https://www.sciencedirect.com/topics/computer-science/instruction-pointer)

На мою думку, ми не можемо замінювати IР на вершину стека, тому що стек відповідає для збереження адреси локальних функції, а ір вказує на наступну інструкцію,яку використовує процесор. Тому це робити не можна, бо стек не має можливості запам'ятовувати послідовність виконання команд.

# Завдання 2.6 (Варіант 9)
## [Код](Pr26.c)
## Опис програми 
...
- На моєму прикладі можемо побачити, як у сегменті BSS зберігається глобальна змінна array1, яка за замовчуванням ініціалізована нулями. Значення, вказане в розділі bss (40032), відповідає пам'яті, зарезервованій для  масиву array1 10000*4байти.
- bss забезпечує мінімізує розмір виконуваного файлу,виділяє пам'ять при запуску програми тта не потребує динамічного розподілу пам’яті.
  ## Висновки
  Сегмент BSS ідеально підходить для зберігання великих масивів, якщо вони не потребують ініціалізації та мають бути доступними протягом усього часу виконання програми. Це дозволяє економити пам’ять у виконуваному файлі та зменшити час виділення пам’яті під час виконання.
