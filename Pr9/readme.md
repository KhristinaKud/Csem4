...
### 2) Напишіть програму, яка виконує команду cat /etc/shadow від імені адміністратора, хоча запускається від звичайного користувача.(Ваша програма повинна робити необхідне, виходячи з того, що конфігурація системи дозволяє отримувати адміністративний доступ за допомогою відповідної команди.)
### [Код](Pr9_2.c)
## Результат компіляції
![](9_2.png)
## Пояснення 
...
![](9_3.png)
## Пояснення 
Ця програма демонструє роботу з файлами та правами доступу. Спочатку вона створює файл original.txt, записує в нього текст і закриває його. Далі перевіряється, чи запущена програма від імені адміністратора (root), інакше робота припиняється. Якщо виконання триває, файл копіюється в домашню директорію під назвою copy.txt, після чого змінюється його власник на root. Потім програма змінює  ідентифікатор користувача на звичайний (seteuid(getuid())) і намагається дописати новий текст у копію файлу. Якщо це не вдається, виводиться повідомлення про помилку. Нарешті, відбувається спроба видалення copy.txt як звичайний користувач. Таким чином, програма демонструє обмеження доступу до файлу залежно від привілеїв користувача.

- Відповіді на питання
Якщо програма спробує змінити файл copy.txt, працюючи як звичайний користувач, то вона, зазнає невдачі. Це пов’язано з тим, що власником файлу після його копіювання та виклику chown() став root. Оскільки звичайний користувач не має прав на запис в цей файл, виклик open може завершитися помилкою, і програма виведе повідомлення про неможливість відкриття файлу для запису.
Далі, якщо програма спробує видалити файл copy.txt , то ця операція також, зазнає невдачі. Видалення файлу можливе лише для його власника або для користувачів, які мають відповідні права доступу до каталогу, в якому розташований файл. 
...
### Варіант №9 Чи можна мати два різні облікові записи з однаковим UID? Які наслідки?
Так, у Unix-подібних операційних системах можна мати два різні облікові записи з однаковим UID (User ID). Це означає, що два логінні імена фактично представлятимуть одного і того ж користувача з точки зору прав доступу та ідентифікації.
Тобто наслідки такі що :
- Обидва користувачі матимуть ті самі дозволи на файли, процеси та команди, оскільки система ідентифікує їх однаково.
- Логи та файли журналів не зможуть точно визначити, який із двох користувачів виконав певну команду, що ускладнює процес, бо не можна зрозуміти хто зробив дію.
- Якщо права дозволяють (chmod), обидва користувачі зможуть змінювати та видаляти файли один одного.
