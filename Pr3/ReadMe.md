...
```
## Пояснення
Встановлюючи ліміт динамічної пам’яті через ulimit -d 1024,  програма завершує  на 909 КБ. Обмеження ulimit поширюється на весь сегмент даних процесу, а не тільки на динамічну пам’ять. Частина цього ліміту використовується під стек, глобальні змінні, код програми та накладні витрати, пов'язані з керуючими структурами malloc. Тому виділена пам’ять через malloc не досягає повного значення обмеження.
